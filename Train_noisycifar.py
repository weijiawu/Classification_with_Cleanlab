...
parser.add_argument('--rate', type=float, default=0.5, help='noise ratio')
...
parser.add_argument('--cv_n_folds', default=10, type=int,
...
parser.add_argument('--frac_noise', default=0.0, type = int,
...
def train(opt,train_idx,train_labels):
    ...
    if train_labels is not None:
        ...
        #                 datasets.ImageFolder(os.path.join(opt.data_path, "train/")).imgs]
        # print(((right_labels!=train_labels)*1).sum()/50000)
    train_loader = torch.utils.data.DataLoader(
        dataset=train_dataset,
        #             sampler=SubsetRandomSampler(train_idx if train_idx is not None else range(MNIST_TRAIN_SIZE)),
        sampler=SubsetRandomSampler(train_idx),
        batch_size=opt.batch_size,
        num_workers=opt.num_classes,
        pin_memory=True
    )
